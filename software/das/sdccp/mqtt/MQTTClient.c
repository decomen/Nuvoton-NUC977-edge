...
#if defined(OPENSSL)
	rc = MQTTProtocol_connect(serverURI, options->intfc, m->c, m->ssl, m->websocket, MQTTVersion, connectProperties, willProperties);
#else
	rc = MQTTProtocol_connect(serverURI, options->intfc, m->c, m->websocket, MQTTVersion, connectProperties, willProperties);
#endif
	if (rc == SOCKET_ERROR)
		goto exit;
