 ...
 *  @return completion code
 */
int Socket_new(const char* addr, size_t addr_len, const char *intfc_name, int port, int* sock)
{
		...
		*sock =	(int)socket(family, type, 0);
		...
		else
		{
            if (intfc_name && intfc_name[0]) {
                ...
                setsockopt(*sock, SOL_SOCKET, SO_BINDTODEVICE, (const void *)&intfc, sizeof(intfc));
            }
