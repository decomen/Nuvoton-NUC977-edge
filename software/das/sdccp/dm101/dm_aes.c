...
#define Nk	(DM_AES_KEY_LENGTH / 32)		// 以“字”（4字节）为单位的密钥长度
#define Nb	4							// 以“字”（4字节）为单位的加解密数据块大小，固定为4

// Nr：加密的轮数
#if   DM_AES_KEY_LENGTH == 128
	#define Nr	10
#elif DM_AES_KEY_LENGTH == 192
	#define Nr	12
#elif DM_AES_KEY_LENGTH == 256
	#define Nr	14
#else
	#error DM_AES_KEY_LENGTH must be 128, 192 or 256 BOOLs!
#endif

// GF(28) 多项式
#define BPOLY 0x1B // Lower 8 BOOLs of (x^8 + x^4 + x^3 + x + 1), ie. (x^4 + x^3 + x + 1).

// DM_AES子密钥表，当密钥长度为128位时，占用176字节空间
				 ...
				 unsigned int nDataLen, const unsigned char *pIV)
{
	...
	{
		#if DM_AES_MODE == DM_AES_MODE_CBC
...
*/
 char *PKCS7_EnPadding(char *pPlainText,unsigned int nDataLen,unsigned int *toalsize) {
	 char *ret = NULL;
	 unsigned int pidding_size = (DM_AES_KEY_LENGTH/8) - (nDataLen % (DM_AES_KEY_LENGTH/8));
	 unsigned int i;
	 //printf("PKCS7_EnPadding nDataLen = %d\n", nDataLen);
	 *toalsize = nDataLen + pidding_size;
	 //printf("PKCS7_EnPadding toalsize = %d\n", *toalsize);
	 if (pidding_size!=0) {
		 for (i =nDataLen;i < (nDataLen+pidding_size); i++ ) {
			 pPlainText[i] = pidding_size;
		 }
	 }
	 return pPlainText;
 }
 ...
 char *PKCS7_DePadding(char *pPlainText,unsigned int nDataLen, unsigned int *toalsize) {
 	 if(pPlainText[nDataLen - 1] <= (DM_AES_KEY_LENGTH/8)){
	 	*toalsize = nDataLen - pPlainText[nDataLen - 1]; 	
 	 }else{
		*toalsize = nDataLen;
	 }
	 return pPlainText;
