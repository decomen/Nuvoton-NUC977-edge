...
HTTP_API const char* ft_http_sync_request(ft_http_client_t* http, const char* url, http_request_method_e m, const char* post_data, int post_data_len)
{
	if(http == NULL)
	{
		return NULL;
	...
	http->error_code = http_internal_sync_request(http, url, post_data, post_data_len, 0, 0);
