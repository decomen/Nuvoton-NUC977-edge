...
typedef struct _modbus_customize_backend {
    ssize_t (*send) (modbus_t *ctx, const uint8_t *req, int req_length);
    ssize_t (*recv) (modbus_t *ctx, uint8_t *rsp, int rsp_length);
    int (*connect) (modbus_t *ctx);
    void (*close) (modbus_t *ctx);
    int (*select) (modbus_t *ctx, fd_set *rset, struct timeval *tv, int msg_length);
} modbus_customize_backend_t;
...
typedef struct _modbus_monitor {
    void (*read) (modbus_t *ctx, const uint8_t *data, int length);
    void (*write) (modbus_t *ctx, const uint8_t *data, int length);
    void (*state_change) (modbus_t *ctx, int s, modbus_state_t state);
} modbus_monitor_t;
...
MODBUS_API int modbus_set_customize_backend(modbus_t *ctx, modbus_customize_backend_t *backend);
MODBUS_API int modbus_set_monitor(modbus_t *ctx, modbus_monitor_t *monitor);
...
#include "modbus-ascii-over-tcp.h"
#include "modbus-rtu-over-udp.h"
#include "modbus-ascii-over-udp.h"
