...
static uint8_t FskReadIoModeReady()
{
	SX1276Read( REG_IRQFLAGS1, &SX1276->RegIrqFlags1);
	return ( (SX1276->RegIrqFlags1 & RF_IRQFLAGS1_MODEREADY) == RF_IRQFLAGS1_MODEREADY);
}

static uint8_t FskReadIoRxReady()
{
	SX1276Read( REG_IRQFLAGS1, &SX1276->RegIrqFlags1);
	return ( (SX1276->RegIrqFlags1 & RF_IRQFLAGS1_RXREADY) == RF_IRQFLAGS1_RXREADY);
}

static uint8_t FskReadIoTxReady()
{
	SX1276Read( REG_IRQFLAGS1, &SX1276->RegIrqFlags1);
	return ( (SX1276->RegIrqFlags1 & RF_IRQFLAGS1_TXREADY) == RF_IRQFLAGS1_TXREADY);
}
static uint8_t FskReadIoPllLock()
{
	SX1276Read( REG_IRQFLAGS1, &SX1276->RegIrqFlags1);
	return ( (SX1276->RegIrqFlags1 & RF_IRQFLAGS1_PLLLOCK) == RF_IRQFLAGS1_PLLLOCK);
}
static uint8_t FskReadIoRssi()
{
	SX1276Read( REG_IRQFLAGS1, &SX1276->RegIrqFlags1);
	return ( (SX1276->RegIrqFlags1 & RF_IRQFLAGS1_RSSI) == RF_IRQFLAGS1_RSSI);
}
static uint8_t FskReadIoTimeout()
{
	SX1276Read( REG_IRQFLAGS1, &SX1276->RegIrqFlags1);
	return ( (SX1276->RegIrqFlags1 & RF_IRQFLAGS1_TIMEOUT) == RF_IRQFLAGS1_TIMEOUT);
}

//当前导码检测器检测到有效 前导码时设置该位。设置为1 时，清除该位。
static uint8_t FskReadIoPreambleDetech()  // 
{
	SX1276Read( REG_IRQFLAGS1, &SX1276->RegIrqFlags1);
	return ( (SX1276->RegIrqFlags1 & RF_IRQFLAGS1_PREAMBLEDETECT) == RF_IRQFLAGS1_PREAMBLEDETECT);
}

//检测到Sync 和Address（如果启动）时，设置该位。

static uint8_t FskReadIoSyncAddressMatch()
{
	SX1276Read( REG_IRQFLAGS1, &SX1276->RegIrqFlags1);
	return ( (SX1276->RegIrqFlags1 & RF_IRQFLAGS1_SYNCADDRESSMATCH) == RF_IRQFLAGS1_SYNCADDRESSMATCH);
}

/*========== REG_IRQFLAGS2 ==========*/

static uint8_t FskReadIoFifoFull()
{
	SX1276Read( REG_IRQFLAGS2, &SX1276->RegIrqFlags2);
	return ( (SX1276->RegIrqFlags2 & RF_IRQFLAGS2_FIFOFULL) == RF_IRQFLAGS2_FIFOFULL);
}

static uint8_t FskReadIoFifoEmpty()
{
	SX1276Read( REG_IRQFLAGS2, &SX1276->RegIrqFlags2);
	return ( (SX1276->RegIrqFlags2 & RF_IRQFLAGS2_FIFOEMPTY) == RF_IRQFLAGS2_FIFOEMPTY);
}
static uint8_t FskReadIoFifoLevel()
{
	SX1276Read( REG_IRQFLAGS2, &SX1276->RegIrqFlags2);
	return ( (SX1276->RegIrqFlags2 & RF_IRQFLAGS2_FIFOLEVEL) == RF_IRQFLAGS2_FIFOLEVEL);
}
static uint8_t FskReadIoFifoOverrun()
{
	SX1276Read( REG_IRQFLAGS2, &SX1276->RegIrqFlags2);
	return ( (SX1276->RegIrqFlags2 & RF_IRQFLAGS2_FIFOOVERRUN) == RF_IRQFLAGS2_FIFOOVERRUN);
}
static uint8_t FskReadIoPacketSent()
{
	SX1276Read( REG_IRQFLAGS2, &SX1276->RegIrqFlags2);
	return ( (SX1276->RegIrqFlags2 & RF_IRQFLAGS2_PACKETSENT) == RF_IRQFLAGS2_PACKETSENT);
}
static uint8_t FskReadIoPayloadReady()
{
	SX1276Read( REG_IRQFLAGS2, &SX1276->RegIrqFlags2);
	return ( (SX1276->RegIrqFlags2 & RF_IRQFLAGS2_PAYLOADREADY) == RF_IRQFLAGS2_PAYLOADREADY);
}
static uint8_t FskReadIoCrcOk()
{
	SX1276Read( REG_IRQFLAGS2, &SX1276->RegIrqFlags2);
	return ( (SX1276->RegIrqFlags2 & RF_IRQFLAGS2_CRCOK) == RF_IRQFLAGS2_CRCOK);
}
static uint8_t FskReadIoLowBat()
{
	SX1276Read( REG_IRQFLAGS2, &SX1276->RegIrqFlags2);
	return ( (SX1276->RegIrqFlags2 & RF_IRQFLAGS2_LOWBAT) == RF_IRQFLAGS2_LOWBAT);
}
...
uint32_t SX1276FskProcess( void )
{
    ...
    case RF_STATE_RX_SYNC:
        if( ( FskReadIoPreambleDetech()/*DIO4 == 1*/ ) && ( PreambleDetected == false ) )// Preamble
        {
        ...
        if( (FskReadIoSyncAddressMatch() /*DIO2 == 1*/ ) && ( PreambleDetected == true ) && ( SyncWordDetected == false ) ) // SyncAddr
        {
...
#if 1
        if( FskReadIoFifoLevel() /*DIO1 == 1*/ ) // FifoLevel
        {
...
#endif
        if( /* FskReadIoPayloadReady()*/ DIO0 == 1 ) // PayloadReady/CrcOk
        {
    ...
    case RF_STATE_TX_READY_WAIT:
        if( FskReadIoTxReady() /* DIO3 == 1*/ )    // TxReady
        {
            if( ( SX1276->RegPacketConfig1 & RF_PACKETCONFIG1_PACKETFORMAT_VARIABLE ) == RF_PACKETCONFIG1_PACKETFORMAT_VARIABLE )
            {
    ...
    case RF_STATE_TX_RUNNING:
        if( FskReadIoFifoLevel() /*DIO1 == 0*/ )    // FifoLevel below thresold
        {  
