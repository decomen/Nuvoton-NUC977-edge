...
static uint8_t LoraReadIoRxTimeout()
{
	SX1276Read( REG_LR_IRQFLAGS, &SX1276LR->RegIrqFlags);
	return ( (SX1276LR->RegIrqFlags & RFLR_IRQFLAGS_RXTIMEOUT) == RFLR_IRQFLAGS_RXTIMEOUT);
}

static uint8_t LoraReadIoRxDone()
{
	SX1276Read( REG_LR_IRQFLAGS, &SX1276LR->RegIrqFlags);
	return ( (SX1276LR->RegIrqFlags & RFLR_IRQFLAGS_RXDONE) == RFLR_IRQFLAGS_RXDONE);
}


static uint8_t LoraReadIoPayloadCrcError()
{
	SX1276Read( REG_LR_IRQFLAGS, &SX1276LR->RegIrqFlags);
	return ( (SX1276LR->RegIrqFlags & RFLR_IRQFLAGS_PAYLOADCRCERROR) == RFLR_IRQFLAGS_PAYLOADCRCERROR);
}

static uint8_t LoraReadIoValidHeader()
{
	SX1276Read( REG_LR_IRQFLAGS, &SX1276LR->RegIrqFlags);
	return ( (SX1276LR->RegIrqFlags & RFLR_IRQFLAGS_VALIDHEADER) == RFLR_IRQFLAGS_VALIDHEADER);
}

static uint8_t LoraReadIoTxDone()
{
	SX1276Read( REG_LR_IRQFLAGS, &SX1276LR->RegIrqFlags);
	return ( (SX1276LR->RegIrqFlags & RFLR_IRQFLAGS_TXDONE) == RFLR_IRQFLAGS_TXDONE);
}

static uint8_t LoraReadIoCadDone()
{
	SX1276Read( REG_LR_IRQFLAGS, &SX1276LR->RegIrqFlags);
	return ( (SX1276LR->RegIrqFlags & RFLR_IRQFLAGS_CADDONE) == RFLR_IRQFLAGS_CADDONE);
}

static uint8_t LoraReadIoFhssChangeChannel()
{
	SX1276Read( REG_LR_IRQFLAGS, &SX1276LR->RegIrqFlags);
	return ( (SX1276LR->RegIrqFlags & RFLR_IRQFLAGS_FHSSCHANGEDCHANNEL) == RFLR_IRQFLAGS_FHSSCHANGEDCHANNEL);
}

static uint8_t LoraReadIoCadDetected()
{
	SX1276Read( REG_LR_IRQFLAGS, &SX1276LR->RegIrqFlags);
	return ( (SX1276LR->RegIrqFlags & RFLR_IRQFLAGS_CADDETECTED) == RFLR_IRQFLAGS_CADDETECTED);
}
...
uint32_t SX1276LoRaProcess( void )
{
    ...
    case RFLR_STATE_RX_RUNNING:
       ...
       // if( DIO2 == 1 ) // FHSS Changed Channel
       	if(LoraReadIoFhssChangeChannel())
        {
            RxTimeoutTimer = GET_TICK_COUNT( );
            if( LoRaSettings.FreqHopOn == true )
            {
